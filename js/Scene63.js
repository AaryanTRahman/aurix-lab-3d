  ...
  return new Promise((resolve, reject) => {
    ...
    function check() {
      ...
      if (performance.now() - start > 5000) {
        reject(new Error('GSAP globals were not available for the Aurix scene.'));
        return;
      }
...
const CAMERA_SCROLL_CONFIG = {
  ...
  mobile: {
    startOffset: new THREE.Vector3(-2.5, -0.85, 9.5),
  ...
  fov:   { start: 50, end: 26, mobileStart: 61, mobileEnd: 55 },
...
let lastHeroViewportWidth = Math.round(window.visualViewport?.width || window.innerWidth || 0);
...
function syncHeroViewportHeight(force = false) {
  ...
  const currentViewportWidth = Math.round(window.visualViewport?.width || window.innerWidth || 0);
  const widthChanged = Math.abs(currentViewportWidth - lastHeroViewportWidth) > 2;
  const currentY = window.scrollY || window.pageYOffset || 0;
  if (!force && !widthChanged && currentY > 8) {
    return;
  }

  const viewportHeight = `${getViewportHeight()}px`;
  [heroSection, container].forEach((element) => {
    if (!element) return;
    element.style.height = viewportHeight;
    element.style.minHeight = viewportHeight;
    element.style.maxHeight = viewportHeight;
  });
  lastHeroViewportWidth = currentViewportWidth;
}

function getViewportSize() {
  syncHeroViewportHeight();
  const width = container?.clientWidth || window.innerWidth;
  const height = container?.clientHeight || getViewportHeight();
...
function resizeScene() {
  ...
  const viewportHeightChanged = Math.abs(height - lastViewportHeight) > 2;
  const canRefreshForHeightChange = viewportHeightChanged && (window.scrollY || window.pageYOffset || 0) <= 8;

  if (window.innerWidth !== lastWindowWidth || canRefreshForHeightChange) {
    lastWindowWidth = window.innerWidth;
    lastViewportHeight = height;
    ScrollTrigger?.refresh();
  } else if (viewportHeightChanged) {
    lastViewportHeight = height;
  }
